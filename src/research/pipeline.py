...
async def run_research(topic: str, *, query_id: int | None = None) -> int:
    ...
    if not sources:
        mark_failed(query_id, "no source adapters available — set TAVILY_API_KEY and/or REDDIT_* and try again")
    ...
    try:
        ...
        mark_failed(query_id, f"planner failed: {exc}")
        raise

    # 3) research — populate ResearcherDeps.docs.
    deps = ResearcherDeps(docs=[], seen_ids=existing_doc_keys())
    try:
        ...
        annotate(query_id, researcher_warning=str(exc)[:500])

    new_doc_ids = persist_documents(query_id, deps.docs)
    log.info("research[%d] collected docs=%d (new in this run=%d)", query_id, len(deps.docs), len(new_doc_ids))

    if not deps.docs:
        mark_failed(query_id, "researcher returned 0 documents — check source credentials")
    ...
    try:
        ...
        mark_failed(query_id, f"synthesizer failed: {exc}")
        raise

    # 5) persist findings + mark done.
    finding_count = persist_findings(query_id, bundle.findings, deps.docs)
    mark_done(query_id, stats={
